...
import { BookOpen, Clock, Search, ChevronRight, Star, TrendingUp, Award, Zap, ArrowLeft, Code, Lightbulb, Target, Bookmark, BookmarkCheck, Filter, X } from 'lucide-react';
import { dsaArticlesEnhanced as dsaArticles, getAllArticles, searchArticles } from '../data/dsaArticlesEnhanced';

const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';

const DSAArticlesViewerWithBookmarks = () => {
  const { user } = useUser();
  ...
  const toggleBookmark = async (article) => {
    ...
    setBookmarkLoading(prev => ({ ...prev, [article.id]: true }));
    ...
    try {
      if (isBookmarked(article.id)) {
        ...
        // Add bookmark
        const response = await fetch(`${API_URL}/api/bookmarks/add`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: user.id,
      ...
      setBookmarkLoading(prev => ({ ...prev, [article.id]: false }));
    }
  };
  ...
  const getFilteredArticles = () => {
    ...
    if (showBookmarksOnly) {
      ...
      articles = articles.filter(a => bookmarkedIds.includes(a.id));
    }
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <button 
                    ...
                    disabled={bookmarkLoading[selectedArticle.id]}
    ...
    <div className={`min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white p-6 overflow-y-auto transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex flex-col md:flex-row gap-4 animate-fade-in" style={{ animationDelay: '200ms' }}>
          ...
          >
            {showBookmarksOnly ? <BookmarkCheck className="w-5 h-5" /> : <Bookmark className="w-5 h-5" />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredArticles.map((article, index) => (
            <div
              key={article.id}
              className="group bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50 hover:border-gray-600 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/10 animate-fade-in-up relative"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <button
                onClick={(e) => {
...
export default DSAArticlesViewerWithBookmarks;
